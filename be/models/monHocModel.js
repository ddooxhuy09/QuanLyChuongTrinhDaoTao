    ...
    async themMonHoc(monHocData) {
        try {
            ...
            if (monHocData.maKhoiKienThuc !== undefined) 
                request.input("MaKhoiKienThuc", sql.VarChar(10), monHocData.maKhoiKienThuc);
            
            if (monHocData.maMonHocTruoc !== undefined) 
                request.input("MaMonHocTruoc", sql.VarChar(15), monHocData.maMonHocTruoc);
            
            if (monHocData.maMonHocTienQuyet !== undefined) 
                request.input("MaMonHocTienQuyet", sql.VarChar(15), monHocData.maMonHocTienQuyet);
            
            if (monHocData.maMonHocSongHanh !== undefined) 
                request.input("MaMonHocSongHanh", sql.VarChar(15), monHocData.maMonHocSongHanh);
            
            if (monHocData.tenMonHocTiengAnh !== undefined) 
                request.input("TenMonHocTiengAnh", sql.NVarChar(150), monHocData.tenMonHocTiengAnh);
            ...
            const errorMessage = error.message || error.originalError?.message || "Lỗi không xác định";
            return {
                success: false,
